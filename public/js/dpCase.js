$( document ).ready(function() {
    ...
    function dpGroupParent(children, index){
        ...
        markup=`<tr class='table-secondary'><td>For The ${index + 1}'s Duplicate<td></tr>`
    ...
    function dpGroupChild(child, index){
        ...
        <td><input type="checkbox" class="messageCheckbox" value="${child.id}"></td>
        <td><a  class="btn btn-outline-primary" data-toggle="modal" data-target="#markRow" onclick="takeCaseId(${child.id})">Mark Duplicate</a></td>
        </tr>`
        tableBody = $("table tbody")
        tableBody.append(markup)
    }
